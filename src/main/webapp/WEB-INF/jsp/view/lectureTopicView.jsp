...
<html>
    ...
    <textContent>
        ...
        <security:authorize access="hasRole('ADMIN')">
            [<a href="<c:url value="/lecture/edit/${topicId}" />">Edit</a>]
            [<a href="<c:url value="/lecture/delete/${topicId}" />">Delete</a>]
        </security:authorize>
        <security:authorize access="isAuthenticated()">
            [<a href="<c:url value="/topic/reply/${topicId}" />">Reply</a>]
        </security:authorize>
        <br /><br />
        ...
        <c:if test="${topic.numberOfAttachments > 0}">
            ...
            <c:forEach items="${topic.attachments}" var="attachment"
                ...
                <a href="
                    <security:authorize access="isAuthenticated()">
                        <c:url value="/lecture/${topicId}/attachment/${attachment.name}" />
                    </security:authorize>
                    <security:authorize access="isAnonymous()">
                        <c:url value="/login" />
                    </security:authorize>
        ...
        <a href="<c:url value="/lecture" />">Return to list topics of lecture</a>
        <br />
        <c:choose>
